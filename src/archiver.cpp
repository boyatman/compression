...
Archiver::Archiver(const std::string& ifile, const std::string& ofile) :
    ifile_(ifile), ofile_(ofile), mds_() {
}

Archiver::ARCHIVER_RETURN_CODE
Archiver::archive() {
  std::fstream isfile;
...
void
Archiver::ProcessSegment(std::size_t index, const std::bitset<10>& bitset, const std::vector<std::string>& row,
  ...
  for (size_t i = 0;i < 10;++i) {
    ...
    if (!bitset.test(i)) {
      if (level.GetNumber() != static_cast<std::uint32_t>(stoi(row[base]))) {
        std::cout << "This row has an issue " << index_ << " on index " << i << " Number(" << level.GetNumber() << ", "
            << static_cast<std::uint32_t>(stoi(row[base])) << ")" << std::endl;
      }
      if (level.GetQuantity() != static_cast<std::uint32_t>(stoi(row[base + 2]))) {
        std::cout << "This row has an issue " << index_ << " on index " << i << " Quantity(" << level.GetQuantity()
            << ", " << static_cast<std::uint32_t>(stoi(row[base + 2])) << ")" << std::endl;
      }
      if (level.GetPrice() - stod(row[base + 1]) > 0.0001) {
        std::cout << "This row has an issue " << index_ << " on index " << i << " Price(" << level.GetPrice() << ", "
...
void
Archiver::ProcessRow(const std::vector<std::string>& row) const {
...
std::bitset<10>
Archiver::ProcessBitset(const std::string& row) const {
  std::bitset<10> bits(row);
  return bits;
}

void
Archiver::ArchiveRow(std::fstream& fs) const {
  ...
  const auto& blevels = mds_.GetBidLevels();
  for (auto i = 0U;i < 10;++i) {
    if (!bbs.test(i)) continue;
    const auto& level = blevels[i];
  ...
  const auto& alevels = mds_.GetAskLevels();
  for (auto i = 0U;i < 10;++i) {
    if (!abs.test(i)) continue;
    const auto& level = alevels[i];
    fs.write(reinterpret_cast<const char*>(&level), sizeof(BookLevel));
  }
}
